...
func MultiPanelUpdate(c *gin.Context) {
	...
	// check panel exists
	if !ok {
		c.JSON(404, utils.ErrorStr("Kein Panel mit der angegebenen ID gefunden"))
		return
	}
	...
	if err != nil {
		...
		if errors.As(err, &unwrapped) && unwrapped.StatusCode == 403 {
			c.JSON(http.StatusBadRequest, utils.ErrorStr("Ich habe keine Berechtigung, Nachrichten in dem angegebenen Kanal zu senden"))
		} else {
			_ = c.AbortWithError(http.StatusInternalServerError, app.NewError(err, "Failed to update multi-panel"))
		}
