...
func GetTicket(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, utils.ErrorStr("Ungültige Ticket ID angegeben: %s", c.Param("ticketId")))
		return
	}
	...
	if ticket.GuildId != guildId {
		c.JSON(http.StatusForbidden, utils.ErrorStr("Ticket #%d gehört nicht zur Guild %d", ticketId, guildId))
		return
	}

	if !ticket.Open {
		c.JSON(http.StatusNotFound, utils.ErrorStr("Ticket #%d ist geschlossen", ticketId))
		return
	}
	...
	if !hasPermission {
		c.JSON(http.StatusForbidden, utils.ErrorStr("Benutzer %d hat keine Berechtigungen dir Transscript #%d anzuschauen", userId, ticketId))
		return
	}

	if ticket.ChannelId == nil {
		c.JSON(http.StatusNotFound, utils.ErrorStr("Ticket #%d Channel nicht gefunden", ticketId))
		return
	}
